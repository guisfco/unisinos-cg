...
int setupGeometry(string objFile, int& verticesSize);
...
int main()
{
	...
	GLuint suzanneVAO = setupGeometry(suzanneObj, suzanneVerticesSize);
	map<string, string> propertiesSuzanne;
	readMaterialsFile(suzanneMtl, propertiesSuzanne);
	GLuint texSuz = loadTexture(propertiesSuzanne["map_Kd"]);

	GLuint cubeVAO = setupGeometry(cubeObj, cubeVerticesSize);
	map<string, string> propertiesCube;
	readMaterialsFile(cubeMtl, propertiesCube);
	GLuint texCube = loadTexture(propertiesCube["map_Kd"]);

	GLuint ballVAO = setupGeometry(ballObj, ballVerticesSize);
	map<string, string> propertiesBall;
	readMaterialsFile(ballMtl, propertiesBall);
	GLuint texBall = loadTexture(propertiesBall["map_Kd"]);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		shader.setFloat("ks", stofOrElse(propertiesSuzanne["Ks"], 0));
		shader.setFloat("q", stofOrElse(propertiesSuzanne["Ns"], 0));

		glDrawArrays(GL_TRIANGLES, 0, suzanneVerticesSize);
		...
		shader.setFloat("kd", stofOrElse(propertiesCube["Kd"], 1.5));
		shader.setFloat("ks", stofOrElse(propertiesCube["Ks"], 0));
		shader.setFloat("q", stofOrElse(propertiesCube["Ns"], 0));
		...
		shader.setFloat("ka", stofOrElse(propertiesBall["Ka"], 0));
		shader.setFloat("kd", stofOrElse(propertiesBall["Kd"], 1.5));
		shader.setFloat("ks", stofOrElse(propertiesBall["Ks"], 0));
		shader.setFloat("q", stofOrElse(propertiesBall["Ns"], 0));
